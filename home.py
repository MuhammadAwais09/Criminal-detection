...
def get_criminals(name):
   
    with app.app_context():
        criminal = Criminal.query.filter_by(name=name).first()
        if criminal:
            return (criminal.id, {
                'Name': criminal.name,
                'Father\'s Name': criminal.father_name,
                'Gender': criminal.gender,
                'DOB(yyyy-mm-dd)': criminal.dob.strftime('%Y-%m-%d'),
                'Crimes Done': criminal.crimes_done
            })
    return (None, {})
...
def create_temp_criminal_folder():
    folder_path = 'face_samples\\temp_criminal'
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

# Call the function to create the folder
create_temp_criminal_folder()


def initModelAndCamera():
    global model, names, webcam
    (model, names) = train_model()
    print('Training Successful. Camera Ready.')
...
def basicPageSetup(pageNo):
    ...
    back_img = tk.PhotoImage(file= r"C:\face detection and behaviour detection\img\back.png")
...
def register(entries, required, menu_var):
    ...
    if(len(no_face) > 0):
        ...
        # Storing data in database using the 'add_criminal()' function
        add_criminal(entry_data)
...
def showCriminalProfile(name):
    ...
    (id, crim_data) = get_criminals(name)
...
def startCameraThread():
    global webcam, thread_event, img_label, left_frame, right_frame
    thread_event = threading.Event()

    # Initialize the model and camera in a separate thread
    model_thread = threading.Thread(target=initModelAndCamera)
    model_thread.start()
...
def getPage3():
    ...
    tk.Button(btn_grid, text="Start Detection", command=startCameraThread, font="Arial 15 bold", padx=20, bg="#000000",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#3E3B3C", activeforeground="white").grid(
...
def selectvideo1():
    ...
    if(len(path) > 0):
       ...
       ### code camera live 
def liveCameraDetection():
    global active_page, webcam, thread_event, heading, left_frame, right_frame, img_label
    active_page = 3
    pages[3].lift()

    basicPageSetup(3)
    heading.configure(text="Live Camera Detection")

    btn_grid = tk.Frame(left_frame, bg="#3E3B3C")
    btn_grid.pack()

    tk.Button(btn_grid, text="Start Detection", command=startLiveDetection, font="Arial 15 bold", padx=20, bg="#000000",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#3E3B3C", activeforeground="white").grid(
        row=0, column=0, padx=25, pady=25)

    tk.Button(btn_grid, text="Stop Detection", command=stopLiveDetection, font="Arial 15 bold", padx=20, bg="#000000",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#3E3B3C", activeforeground="white").grid(
        row=0, column=1, padx=25, pady=25)

def startLiveDetection():
    global webcam, thread_event, img_label, left_frame, right_frame
    ...
    thread_event = threading.Event()
    thread = threading.Thread(target=liveDetectionLoop, args=(model, names))
    thread.start()

def stopLiveDetection():
...
def liveDetectionLoop(model, names):
    global thread_event, left_frame, right_frame, img_label, webcam

    webcam = cv2.VideoCapture(0)  # Use default camera (you can change the camera index if needed)
    old_recognized = []
    crims_found_labels = []
    times = []
    img_label = None
    ...
    try:
        while not thread_event.is_set():
            # Loop until the thread_event is set
...
######################################## Home Page ####################################
tk.Label(pages[0], text="Face Recognition System for Criminal Detection", fg="black", bg="#355E3B",
      font="Arial 25 bold", pady=30).pack()

logo = tk.PhotoImage(file = r"C:\face detection and behaviour detection\img\logo2.png")
tk.Label(pages[0], image=logo, bg="#FFFFFF").pack(side='left')

btn_frame = tk.Frame(pages[0], bg="#3E3B3C", pady=30)
btn_frame.pack()

tk.Button(btn_frame, text="Input Video", command=selectvideo)
...
tk.Button(btn_frame, text="Live Camera Detection", command=liveCameraDetection)
